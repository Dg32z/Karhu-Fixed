...
implements KarhuHandler {
    private final KarhuPlayer data;
    private List<Block> cache1_13 = new CopyOnWriteArrayList<Block>();
    ...
    public void handle(boolean moved) {
        ...
        BoundingBox mcpAnyLiquidBox = this.data.getBoundingBox().clone().translate(this.data.deltas.deltaX, this.data.deltas.lastMotionY * (double)0.8f + (double)0.6f, this.data.deltas.deltaZ);
        ...
        KarhuStream<Material> blocksBelow = new KarhuStream<Material>(below.getCachedCollidingBlocks(tickCache));
        KarhuStream<Material> blocksNearHugeBelow = new KarhuStream<Material>(belowHugeBox.getCachedCollidingBlocks(tickCache));
        KarhuStream<Material> blocksInsideMiddleBox = new KarhuStream<Material>(inside.getCachedCollidingBlocks(tickCache));
        KarhuStream<Material> normalBox = new KarhuStream<Material>(this.data.getBoundingBox().getCachedCollidingBlocks(tickCache));
        KarhuStream<Material> lqStream = new KarhuStream<Material>(lqBelow.getCachedCollidingBlocks(tickCache));
        KarhuStream<Material> mcpLandStream = new KarhuStream<Material>(this.data.getBoundingBox().clone().getCollidingOnLanded(tickCache, MathHelper.floor_double(this.data.getLocation().getX()), MathHelper.floor_double(this.data.getLocation().getY() - (double)0.2f), MathHelper.floor_double(this.data.getLocation().getZ())));
        KarhuStream<Material> mcpWaterStream = new KarhuStream<Material>(mcpWaterBox.getCollidingMaterialAccel(tickCache));
        KarhuStream<Material> mcpLavaStream = new KarhuStream<Material>(mcpLavaBox.getCollidingMaterialAccel(tickCache));
        KarhuStream<Material> blocksHori = new KarhuStream<Material>(horiBox.getCachedCollidingBlocks(tickCache));
        KarhuStream<Material> blocksAbove = new KarhuStream<Material>(above.getCachedCollidingBlocks(tickCache));
        KarhuStream<Material> blocksAboveStrict = new KarhuStream<Material>(aboveStrict.getCachedCollidingBlocks(tickCache));
        KarhuStream<Entity> entities = null;
        KarhuStream<com.github.retrooper.packetevents.protocol.entity.type.EntityType> entities2 = null;
        if (Karhu.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19)) {
            entities = new KarhuStream<Entity>(this.data.getBoundingBox().clone().expand(0.3, 0.6, 0.3).getCollidingEntities());
        } else {
            entities2 = new KarhuStream<com.github.retrooper.packetevents.protocol.entity.type.EntityType>(this.data.getBoundingBox().clone().expand(0.3, 0.6, 0.3).getCollidingEntitiesNew());
        }
        KarhuStream<Entity> entitiesUp = null;
        KarhuStream<com.github.retrooper.packetevents.protocol.entity.type.EntityType> entitiesUp2 = null;
        if (Karhu.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19)) {
            entitiesUp = new KarhuStream<Entity>(this.data.getBoundingBox().clone().expandMin(0.0, 1.5, 0.0).expandMax(0.0, 0.7, 0.0).expand(0.65, 0.0, 0.65).getCollidingEntities());
        } else {
            entitiesUp2 = new KarhuStream<com.github.retrooper.packetevents.protocol.entity.type.EntityType>(this.data.getBoundingBox().clone().expandMin(0.0, 1.5, 0.0).expandMax(0.0, 0.7, 0.0).expand(0.65, 0.0, 0.65).getCollidingEntitiesNew());
        }
        ...
        boolean entity = Karhu.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19) ? entities.any(e -> e.getType() == EntityType.BOAT) : entities2.any(e -> EntityTypes.BOAT.equals(e) || EntityTypes.CHEST_BOAT.equals(e));
        boolean livingEntity = Karhu.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19) ? entities.any(e -> e instanceof LivingEntity) : entities2.any(e -> EntityTypes.isTypeInstanceOf(e, EntityTypes.LIVINGENTITY));
        ...
        if (this.data.isNewerThan13()) {
            ...
            BoundingBox bbCrouch = new BoundingBox(this.data, location.x - (double)width, location.y, location.z - (double)width, location.x + (double)width, location.y + (double)(height = Boxes.CROUCH.getHeight()), location.z + (double)width);
        ...
        boolean bl = waterBelow = water && !lily;
        if (!this.data.isInUnloadedChunk()) {
        ...
        boolean boatsUp = Karhu.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19) ? entitiesUp.any(e -> e.getType() == EntityType.BOAT) : entitiesUp2.any(e -> EntityTypes.BOAT.equals(e) || EntityTypes.CHEST_BOAT.equals(e));
        ...
        this.data.setInsideBlock(blocksInsideMiddleBox.any(b -> (b.isSolid() || MaterialChecks.WEIRD_SOLID_NO_LIQUID.contains(b)) && !MaterialChecks.SIGNS.contains(b) && !MaterialChecks.CLIMBABLE.contains(b)));

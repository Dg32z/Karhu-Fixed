...
implements ICrashHandler {
    ...
    public void handleFlying(boolean moved, boolean looked, CustomLocation location, CustomLocation lastLocation) {
        this.fPackets += 1.0;
        int n = this.data.getClientVersion().isOlderThan(ClientVersion.V_1_9) ? 300 : 900;
        if (fPackets > (double)n && this.shouldPunish("Move spam")) {
    ...
    public boolean shouldPunish(String type) {
        if (Karhu.getInstance().getConfigManager().isAnticrash()) {
            switch (type) {
                case "Place": {
                    if (Karhu.getInstance().getConfigManager().isPlaceSpam()) break;
                    return true;
                }
                case "Arm": {
                    if (Karhu.getInstance().getConfigManager().isArmSpam()) break;
                    return true;
                }
                case "Slot": {
                    if (Karhu.getInstance().getConfigManager().isSlotSpam()) break;
                    return true;
                }
                case "Window": {
                    if (Karhu.getInstance().getConfigManager().isWindowSpam()) break;
                    return true;
                }
                case "Large move": {
                    if (Karhu.getInstance().getConfigManager().isLargeMove()) break;
                    return true;
                }
                case "Move spam": {
                    if (Karhu.getInstance().getConfigManager().isMoveSpam()) break;
                    return true;
                }
                case "Payload": {
                    if (Karhu.getInstance().getConfigManager().isPayloadSpam()) break;
                    return true;
                }
                default: {
                    return false;

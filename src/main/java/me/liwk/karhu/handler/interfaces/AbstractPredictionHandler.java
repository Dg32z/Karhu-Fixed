...
public abstract class AbstractPredictionHandler {
    ...
    protected final void init() {
        ...
        this.motZ = 0.0;
        this.motX = 0.0;
        HashMap<Double, Pair<Float, Float>> dataAssessments = new HashMap<Double, Pair<Float, Float>>();
        for (float[] floats : NMSValueParser.KEY_COMBOS) {
            ...
            dataAssessments.put(MathUtil.hypot(this.data.deltas.deltaX - this.motX, this.data.deltas.deltaZ - this.motZ), new Pair<Float, Float>(Float.valueOf(strafe), Float.valueOf(forward)));
            this.motX = 0.0;
            this.motZ = 0.0;
        }
        this.motX = x;
    ...
    public static AbstractPredictionHandler loadPredictionHandler(KarhuPlayer data, Karhu karhu) {
        ...
        if (data.getClientVersion() != null) {
            AbstractPredictionHandler abstractPredictionHandler = p = data.isNewerThan8() ? new PredictionHandler1_9(data, karhu) : new PredictionHandler(data, karhu);
        }
        if (p == null) {

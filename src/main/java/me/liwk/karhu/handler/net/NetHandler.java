...
public final class NetHandler {
    ...
    private final Queue<KarhuTask> postPendingTask = new ConcurrentLinkedQueue<KarhuTask>();
    ...
    public void handleServerTransaction(short id, long now) {
        if (id < 0) {
            ...
            TaskData taskData = new TaskData((int)id, now);
    ...
    private void handleInvalidTransaction(short id, short first) {
        if (this.configManager.isNethandler() && this.configManager.isDelay() && !this.kicked) {
            this.kicked = true;
            Tasker.run(() -> this.data.getBukkitPlayer().kickPlayer(this.configManager.getOrderKick().replaceAll("%first%", String.valueOf(first)).replaceAll("%received%", String.valueOf(id))));

...
public final class MiscellaneousAlertPoster {
    ...
    public static void postMiscTp(BaseComponent hover) {
        Karhu.getInstance().getAlertsManager().getMiscDebugToggled().stream().map(Bukkit::getPlayer).filter(Objects::nonNull).forEach(admin -> admin.spigot().sendMessage(hover));
    }
    ...
    public static void handleDiscord(String data, KarhuPlayer pdata) {
        block5: {
            ...
            if (showIcon) {
                discord.addEmbed(new Webhook.EmbedObject().setTitle("```" + player.getName() + "``` | Crash ").setThumbnail("https://minotar.net/avatar/" + player.getName() + "/50.png").setDescription(ChatColor.stripColor((String)data.replaceAll("\n", ""))).setColor(Color.ORANGE).addField("Info", (showWorld ? "World: " + player.getWorld().getName() + " | Coords: " + MiscellaneousAlertPoster.format(1, player.getLocation().getX()) + "/" + MiscellaneousAlertPoster.format(1, player.getLocation().getY()) + "/" + MiscellaneousAlertPoster.format(1, player.getLocation().getZ()) : " Coords: " + MiscellaneousAlertPoster.format(1, player.getLocation().getX()) + "/" + MiscellaneousAlertPoster.format(1, player.getLocation().getY()) + "/" + MiscellaneousAlertPoster.format(1, player.getLocation().getZ())) + (showStats ? " | TPS: " + Karhu.getInstance().getTPS() + " | Ping: " + pdata.getTransactionPing() + "ms" : ""), false).addField("Date", dtf.format(now), false));
            } else {

...
public class LocalStorage
        ...
        new Thread(() -> {
            while (Karhu.getInstance() != null && Karhu.getInstance().getPlug().isEnabled()) {
                try {
                    ...
                    if (this.violations.isEmpty() && this.bans.isEmpty() && this.banWaveQueue.isEmpty()) continue;
                    ...
                    if (this.banWaveQueue.isEmpty()) continue;
    ...
    public List<ViolationX> getViolations(String uuid, Check type, int page, int limit, long from, long to) {
        SQLite.use();
        ArrayList<ViolationX> violations = new ArrayList<ViolationX>();
        Query.prepare("SELECT `MODULE`, `VL`, `TIME`, `EXTRA`, `COORDS`, `WORLD`, `PING`, `TPS` FROM `ALERTS` WHERE `UUID` = ? ORDER BY `TIME` DESC LIMIT ?,?").append(uuid).append(page * limit).append(limit).execute(rs -> violations.add(new ViolationX(uuid, rs.getString(1), rs.getInt(2), rs.getLong(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getLong(7), rs.getDouble(8))));
        return violations;
    }
    ...
    public List<ViolationX> getAllViolations(String uuid) {
        SQLite.use();
        ArrayList<ViolationX> violations = new ArrayList<>();
        Query.prepare("SELECT `MODULE`, `VL`, `TIME`, `EXTRA`, `COORDS`, `WORLD`, `PING`, `TPS` FROM `ALERTS` WHERE `UUID` = ? ORDER BY `TIME`").append(uuid).execute(rs -> violations.add(new ViolationX(uuid, rs.getString(1), rs.getInt(2), rs.getLong(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getLong(7), rs.getDouble(8))));
        return violations;
    }
    ...
    public List<BanX> getRecentBans() {
        ...
        Query.prepare("SELECT `UUID`, `MODULE`, `TIME`, `EXTRA`, `PING`, `TPS` FROM `BANS` ORDER BY `TIME`").execute(rs -> bans.add(new BanX(rs.getString(1), rs.getString(2), rs.getLong(3), rs.getString(4), rs.getLong(5), rs.getDouble(6))));
    ...
    public int getViolationAmount(String uuid) {
        ...
        Query.prepare("SELECT `MODULE`, `VL`, `TIME`, `EXTRA`, `COORDS`, `WORLD`, `PING`, `TPS` FROM `ALERTS` WHERE `UUID` = ? ORDER BY `TIME`").append(uuid).execute(rs -> violations.incrementAndGet());
    ...
    public int getAllViolationsInStorage() {
        ...
        Query.prepare("SELECT `MODULE`, `VL`, `TIME`, `EXTRA`, `COORDS`, `WORLD`, `PING`, `TPS` FROM `ALERTS`").execute(rs -> violations.add(new ViolationX("s", rs.getString(1), rs.getInt(2), rs.getLong(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getLong(7), rs.getDouble(8))));

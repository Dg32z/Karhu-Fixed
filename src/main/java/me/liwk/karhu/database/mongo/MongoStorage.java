...
implements Storage {
    private final CodecRegistry pojoCodecRegistry = CodecRegistries.fromRegistries(MongoClient.getDefaultCodecRegistry(), CodecRegistries.fromProviders(PojoCodecProvider.builder().automatic(true).build()));
            ...
            while (Karhu.getInstance() != null && Karhu.getInstance().getPlug().isEnabled()) {
                try {
                    ...
                    if (!this.violations.isEmpty()) {
                        try {
                            this.loggedViolations.insertMany(new ArrayList<ViolationX>(this.violations));
                        }
                    ...
                    if (!this.bans.isEmpty()) {
                        try {
                            this.loggedBans.insertMany(new ArrayList<BanX>(this.bans));
                        }
                    ...
                    try {
                        this.loggedBanwavePlayers.insertMany(new ArrayList<BanWaveX>(this.banWaveQueue));
                    }
    ...
    public void setAlerts(String uuid, int status) {
        this.loggedStatus.replaceOne(Filters.eq("player", (Object)uuid), new AlertsX(uuid, status));
    }

    @Override
    public boolean getAlerts(String uuid) {
        AlertsX alertsX = this.loggedStatus.find(Filters.eq("player", (Object)uuid)).limit(1).first();
        if (alertsX != null) {
            return MathUtil.getIntAsBoolean(alertsX.status);
        }
        this.loggedStatus.replaceOne(Filters.eq("player", (Object)uuid), new AlertsX(uuid, 1));
        return true;
    }

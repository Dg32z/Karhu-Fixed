...
import me.liwk.karhu.api.event.impl.*;
...
public final class APICaller {
    ...
    public static boolean callAlert(Player player, CheckInfo chk, Check check, String debug, BaseComponent hover, int violations, int maxvl, long ping) {
        ...
        try {
            sheesh = KarhuAPI.getEventRegistry().fireEvent(new KarhuAlertEvent(player, APICaller.toCheckData(check), new DebugData(debug, hover, ping, maxvl), violations));
        }
    ...
    public static boolean callBan(Player player, CheckInfo chk, Check check) {
        boolean sheesh = false;
        try {
            sheesh = KarhuAPI.getEventRegistry().fireEvent(new KarhuBanEvent(player, APICaller.toCheckData(check)));
        }
        catch (Exception exception) {
            // empty catch block
        }
    ...
    public static void callPostCheck(Player player, CheckInfo chk, Check check, Object packet) {
        try {
            KarhuAPI.getEventRegistry().fireEvent(new KarhuPostCheckEvent(check.isDidFail(), APICaller.toCheckData(check), player, packet));
        }
        catch (Exception exception) {
            // empty catch block
        }
    ...
    public static boolean callPreCheck(CheckInfo chk, Check check, Player player, Object packet) {
        boolean sheesh = false;
        try {
            sheesh = KarhuAPI.getEventRegistry().fireEvent(new KarhuPreCheckEvent(player, APICaller.toCheckData(check), packet));
        }
        catch (Exception exception) {
            // empty catch block
        }
    ...
    public static boolean callPullback(Player player, CheckInfo chk, Check check, Location to) {
        return KarhuAPI.getEventRegistry().fireEvent(new KarhuPullbackEvent(player, APICaller.toCheckData(chk), to));
    }

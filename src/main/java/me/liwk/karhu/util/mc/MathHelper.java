...
public class MathHelper {
   ...
   static {
      ...
      for(int j = 0; j < 4096; ++j) {
         SIN_TABLE_FAST[j] = (float)Math.sin(((float)j + 0.5F) / 4096.0F * (float) (Math.PI * 2));
      }

      for(int l = 0; l < 360; l += 90) {
         SIN_TABLE_FAST[(int)((float)l * 11.377778F) & 4095] = (float)Math.sin((float)l * (float) (Math.PI / 180.0));
      }

...
/*
   public boolean checkBlocks(Predicate<Material> p) {
      ...
      int yCeil = (int)Math.ceil(this.maxY);
      int zFloor = (int)Math.floor(this.minZ);
      int zCeil = (int)Math.ceil(this.maxZ);

      for(int x = xFloor; x < xCeil; ++x) {
         for(int y = yFloor; y < yCeil; ++y) {
            for(int z = zFloor; z < zCeil; ++z) {
               Location loc = new Location(this.data.getWorld(), (double)x, (double)y, (double)z);

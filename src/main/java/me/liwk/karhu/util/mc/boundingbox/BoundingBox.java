...
implements Cloneable {
    ...
    public double distance(Location location) {
        return Math.sqrt(Math.min(FastMath.pow((double)(location.getX() - this.minX), (int)2), FastMath.pow((double)(location.getX() - this.maxX), (int)2)) + Math.min(FastMath.pow((double)(location.getZ() - this.minZ), (int)2), FastMath.pow((double)(location.getZ() - this.maxZ), (int)2)));
    }

    public double distance(double x, double z) {
        double dx = Math.min(FastMath.pow((double)(x - this.minX), (int)2), FastMath.pow((double)(x - this.maxX), (int)2));
        double dz = Math.min(FastMath.pow((double)(z - this.minZ), (int)2), FastMath.pow((double)(z - this.maxZ), (int)2));
        return Math.sqrt(dx + dz);
    }

    public double distance(BoundingBox box) {
        double dx = Math.min(FastMath.pow((double)(box.minX - this.minX), (int)2), FastMath.pow((double)(box.maxX - this.maxX), (int)2));
        double dz = Math.min(FastMath.pow((double)(box.minZ - this.minZ), (int)2), FastMath.pow((double)(box.maxZ - this.maxZ), (int)2));
        return Math.sqrt(dx + dz);
    }
    ...
    public void initChunkData() {
        ...
        for (int i1 = i; i1 <= j; ++i1) {
            for (int j1 = k; j1 <= l; ++j1) {
                ...
                long chunkPair = BlockUtil.getChunkPair(test);
                if (chunkPair == lastChunkPair) continue;
    ...
    public List<Entity> getCollidingEntities() {
        ...
        if (!this.nearbyEntities.isEmpty()) {
            for (Entity e : this.nearbyEntities) {
                AxisAlignedBB bb;
                if (e == null || e.getEntityId() == this.data.getBukkitPlayer().getEntityId() || (bb = CollisionBoxParser.from(e)) == null || e == this.data.getBukkitPlayer() || !(bb.distanceXYZ(bbThis) <= 4.0)) continue;
                list.add(e);
            }
        }
        return list;
    ...
    public List<EntityType> getCollidingEntitiesNew() {
        ...
        for (EntityData edata : this.data.getEntityData().values()) {
            AxisAlignedBB bb;
            if (edata.getEid() == this.data.getBukkitPlayer().getEntityId() || (bb = edata.getEntityBoundingBox()) == null || !(bb.distanceXYZ(bbThis) <= 4.0)) continue;
        ...
        for (int x = xFloor; x <= xCeil; ++x) {
            for (int z = zFloor; z <= zCeil; ++z) {
                ...
                for (int y = yFloor - 1; y <= yCeil; ++y) {
                    ...
                    if (block2 == null || MaterialChecks.AIR.contains(block2.getType())) continue;
                    blocks.add(block2);
                }
            }
        }
    ...
    public List<Material> getCachedCollidingBlocks(List<Block> blocks) {
        ArrayList<Material> blocksWanted = new ArrayList<Material>();
        for (Block block : blocks) {
            float expandY;
            float f = expandY = !MaterialChecks.FENCES.contains(block.getType()) && !MaterialChecks.MOVABLE.contains(block.getType()) ? 1.0f : 1.5f;
            AxisAlignedBB blockAABB = new AxisAlignedBB(block.getLocation(), 1.0, expandY);
            if (!this.intersectsWith(blockAABB)) continue;
    ...
    public List<Material> getCollidingOnLanded(List<Block> blocks, double posX, double posY, double posZ) {
        ...
        for (Block block : blocks) {
            ...
            if (!this.intersectsWith(posX, posY, posZ, blockAABB)) continue;
            blocksWanted.add(block.getType());
        }
        return blocksWanted;
    }
            ...
            for (int k1 = i; k1 < j; ++k1) {
                for (int l1 = k; l1 < l; ++l1) {
                    for (int i2 = i1; i2 < j1; ++i2) {
                        if (!this.intersectsWith(k1, l1, i2, blockAABB)) continue;
                        blocksWanted.add(block.getType());
                    }
                }
            }

...
extends PacketCheck {
            ...
            if (packet instanceof FlyingEvent) {
                ...
                if (!this.data.isPossiblyTeleporting()) {
                    for (int target : this.data.getLastTargets()) {
                        ...
                        if (edata != null) {
                            if (!this.data.isNewerThan8()) continue;
                            ...
                            if (Math.abs(this.data.getLocation().pitch) != 90.0f) {
                                dist /= Math.cos(Math.toRadians(this.data.getLocation().pitch));
                            }
                            if (dist > 4.0) {
    ...
    private void handleReach(boolean look) {
        for (int target : this.data.getLastTargets()) {
            ...
            ArrayList<Vec3> rotationVectors = new ArrayList<Vec3>();
            if (look) {
            ...
            double angle = MathUtil.getAngleDistance(this.data.getLocation().getYaw(), direction);

...
extends PositionCheck {
    ...
    public void handle(MovementUpdate update) {
        ...
        if (this.data.getVelocityY() > 0.0) {
            max = (float)Math.max((double)max, this.data.getVelocityY() + (double)0.4f);
        }
        ...
        boolean bl = valid = !this.data.isOnPiston() && !this.data.isWasOnSlime() && !this.data.isPossiblyTeleporting() && !this.data.isSpectating() && this.data.elapsed(this.data.getLastPistonPush()) > 2 && this.data.levitationLevel == 0 && this.data.elapsed(this.data.getLastRiptide()) > 30 && this.data.elapsed(this.data.getLastGlide()) > 30 && this.data.elapsed(this.data.getLastFlyTick()) > 40;
        ...
        if (offsetY > (double)max && valid) {
            this.violations += offsetY - (double)max + 0.5;
            if (violations > 1.0) {
                this.fail("* Jumping higher than expected\n §f* D: §b" + this.format(3, offsetY) + "\n §f* M: §b" + max + "\n §f* TP: §b" + this.data.getTeleportManager().teleportTicks, this.getBanVL(), 300L);
            }
        } else {
            this.violations *= 0.1;
        }
        if (!update.from.ground && update.fromFrom.ground && valid && this.checkBlocks()) {
            ...
            if (difference > 0.0 && offsetY > 0.0 && lastOffsetY > 0.0 && combined > (double)max && !this.data.isTakingVertical()) {
                this.fail("* Jumping higher than expected (V2)\n §f* D: §b" + this.format(3, offsetY) + "\n §f* M: §b" + max + "\n §f* DIFF: §b" + this.format(3, difference) + "\n §f* TP: §b" + this.data.getTeleportManager().teleportTicks, this.getBanVL(), 300L);
            }
        }
        if (this.checkBlocks() && valid && offsetY > 0.0 && lastOffsetY > 0.0 && update.isGround() && !this.data.isTakingVertical()) {
            this.fail("* Impossible upwards movement (GROUND)\n §f* D/LD: §b" + this.format(3, offsetY) + "/" + this.format(3, lastOffsetY), this.getBanVL(), 300L);
        }
    }

    public boolean checkBlocks() {
        return this.data.elapsed(this.data.getLastOnSlime()) > 1 && this.data.elapsed(this.data.getLastOnHalfBlock()) > 3 && this.data.elapsed(this.data.getLastFence()) > 2 && this.data.elapsed(this.data.getLastInPowder()) > 1 && this.data.elapsed(this.data.getLastOnBoat()) > 2;
    }
}

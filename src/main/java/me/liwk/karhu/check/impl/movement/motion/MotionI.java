...
extends PositionCheck {
    ...
    public void handle(MovementUpdate update) {
        ...
        double d = minimum = this.data.getMoveTicks() <= 1 ? -0.2 : -0.1;
        if (!(this.data.getTotalTicks() <= 40 || this.data.isPossiblyTeleporting() || this.data.isRiding() || this.data.isInBed() || this.data.isLastInBed() || this.data.elapsed(this.data.getLastFlyTick()) <= 30 || this.data.elapsed(this.data.getLastPistonPush()) <= 30)) {
            if (motionY < -3.92005) {
                String data = "* Too high vertical motion downwards\n§f* motY: §b" + this.format(3, motionY) + "\n§f* vehicle: §b" + this.data.isRiding() + "\n§f* teleport §b" + this.data.getTeleportManager().teleportTicks;

...
public final class BadPacketsM
        extends PacketCheck {
   Deque<Integer> interactions = new ArrayDeque<Integer>();
   int flying;
   boolean released;
   ...
   public void handle(Event packet) {
      if (this.data.getClientVersion().getProtocolVersion() <= 47) {
         if (packet instanceof FlyingEvent) {
            ...
            if (((ActionEvent)packet).getAction() != WrapperPlayClientEntityAction.Action.START_SPRINTING) {
               return;
            }
            if (this.released) {
               return;

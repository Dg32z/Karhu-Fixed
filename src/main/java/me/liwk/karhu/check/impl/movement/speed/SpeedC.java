...
extends PositionCheck {
                            ...
                            for (boolean sprinting : BOOLEANS) {
                                for (boolean sneaking : BOOLEANS_REVERSED) {
                                    for (boolean jump : BOOLEANS_REVERSED) {
                                        ...
                                        float forward = forw;
                                        float strafe = stra;
                                        if (attack && (this.data.getLastAttackTick() > 2 || this.data.getLastTarget() == null)) continue;
                                        if (sneaking) {
                                        ...
                                        lastDX *= this.data.isLastLastOnGroundPacket() ? (double)lastTickFriction : (double)0.91f;
                                        lastDZ *= this.data.isLastLastOnGroundPacket() ? (double)lastTickFriction : (double)0.91f;
                                        if (attack) {
                                            lastDX *= 0.6;
                                            lastDZ *= 0.6;
                                        }
                                        ...
                                        if (onGround) {
                                            ...
                                            if (jump && sprinting) {
                                                float radians = e.to.yaw * ((float)Math.PI / 180);
                                                lastDX -= (double)(MathHelper.sin(radians) * 0.2f);
                                                lastDZ += (double)(MathHelper.cos(radians) * 0.2f);
                                            }
                                        } else {
                                            float f = f5 = sprinting ? 0.026f : 0.02f;
                                        }
                                        if ((inputForce = forward * forward + strafe * strafe) >= 1.0E-4f) {
                                            if ((inputForce = MathHelper.sqrt_float(inputForce)) < 1.0f) {
                                                inputForce = 1.0f;
                                            }
                                            ...
                                            lastDX += (double)((strafe *= inputForce) * yawCos - (forward *= inputForce) * yawSin);
                                            lastDZ += (double)(forward * yawCos + strafe * yawSin);
                                        }
                                        double[] dArray = new double[]{this.data.deltas.deltaX - lastDX, this.data.deltas.deltaZ - lastDZ};
                                        double moveDiff = MathUtil.hypot(dArray);
    ...
    private double getLeniency() {
        boolean entity;
        double leniency = 0.0;
        boolean bl = entity = this.data.elapsed(this.data.getLastCollidedWithEntity()) <= 10;

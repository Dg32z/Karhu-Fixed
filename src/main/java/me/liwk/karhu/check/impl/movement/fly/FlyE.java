...
extends PacketCheck {
    ...
    public void handle(Event p) {
        if (p instanceof FlyingEvent && ((FlyingEvent)p).hasMoved()) {
            if (!(this.data.elapsed(this.data.getLastRiptide()) < 5 || this.data.isPossiblyTeleporting() || this.data.isInUnloadedChunk() || this.data.isSpectating() || this.data.isOnPiston() || this.data.isOnFence() || this.data.isOnLava() || this.data.isWasOnFence() || this.data.elapsed(this.data.getLastPistonPush()) <= 2 || this.data.getLevitationLevel() != 0 || this.data.getSlowFallingLevel() != 0 || this.data.elapsed(this.data.getLastOnBed()) <= 5 || this.data.elapsed(this.data.getLastInBerry()) <= 1 || this.data.elapsed(this.data.getLastGlide()) <= 140 || this.data.isOnScaffolding() || this.data.getGameMode() == GameMode.CREATIVE || this.data.isInWeb() || this.data.isWasInWeb() || this.data.isWasWasInWeb() || this.data.isLastLadder() || !this.data.isOnLadder())) {
                boolean executePreds = ((this.data.elapsed(this.data.getLastCollidedV()) > 3 && !this.data.isUnderGhostBlock()) || this.data.getClientAirTicks() > 10) && !this.data.isInsideBlock();
                ...
                double pred = this.data.getVelocityYTicks() == 0 ? this.data.getVelocityY() : (lastMotionY - 0.08) * (double)0.98f;
                ...
                if (executePreds) {
                    if (this.data.isOnLadder() || this.data.isLastLadder()) {
                        ...
                        if (this.data.isJumped()) {
                            pred = PlayerUtil.getJumpHeight(this.data);
                            goForward = false;
                        ...
                        if (goForward) {
                            ...
                            if (motionY > 0.07 && this.data.getClientAirTicks() > 3) {
                    ...
                    if (this.data.isWasOnWater()) {
                        ...
                        if (motionY > 0.0) {
                            fixedLastMotion = lastMotionY + (double)0.04f;
                        }
                        pred = this.data.getVelocityYTicks() == 0 ? this.data.getVelocityY() : fixedLastMotion * (double)0.8f - 0.02;
                        threshold += 0.6;
                    ...
                    if (ratio >= threshold && Math.abs(pred) > 0.03 + clamp && this.data.elapsed(this.data.getLastOnSlime()) > 1 && !this.data.isOnBoat() && this.data.getClientAirTicks() > 2 && (this.data.getAirTicks() > 2 || ignoreServerGround)) {
                        this.violations += 1.0 + ratioAndIdcEtc;
                        if (violations > maxiumViolations) {
                ...
                double maxSlime;
                if (this.data.isWasSlimeLand() && this.data.elapsed(this.data.getLastPistonPush()) >= 5 && !this.data.isOnPiston() && motionY >= (maxSlime = Math.max(Math.abs(lastMotionY), Math.abs(lastLastMotionY) + 0.2)) + Math.abs(this.data.getVelocityY()) && motionY > jumpHeight + (double)0.2f) {

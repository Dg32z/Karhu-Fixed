...
public final class TimerC
extends PacketCheck {
    private final Deque<Long> packets = new LinkedList<Long>();
            ...
            if (this.lastFlyingPacket != null) {
                ...
                if (this.packets.size() >= 50) {
                    ...
                    if (timer > speed && this.data.getTotalTicks() > 200) {
                        ...
                        this.violations = now - this.lastFlag > 12500L ? Math.max(this.violations - 1.25, -2.0) : Math.max(this.violations - 0.65, -1.0);
                    }
                    this.packets.clear();
                }
            }

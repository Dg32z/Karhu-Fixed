...
extends PositionCheck {
    ...
    public void handle(MovementUpdate e) {
        ...
        if (!(Math.abs(motionY + 0.098) <= 1.0E-5)) {
            double maxVL;
            double fixedLastMotion;
            double prediction;
            double d = prediction = Math.abs((lastMotionY - 0.08) * (double)0.98f) <= threshold ? 0.0 : (lastMotionY - 0.08) * (double)0.98f;
            if (tickVel != null) {
            ...
            if (this.data.isWasOnWater()) {
                fixedLastMotion = lastMotionY;
                if (motionY > 0.0) {
                    fixedLastMotion = lastMotionY + (double)0.04f;
                }
                prediction = this.data.getVelocityYTicks() == 0 ? this.data.getVelocityY() : fixedLastMotion * (double)0.8f - 0.02;
            ...
            double d2 = maxVL = tickVel != null ? 4.5 : 2.5;
            ...
            } else if (!e.to.ground && !e.from.ground && this.data.getAirTicks() > 0) {
                this.violations += 1.0;
                if (violations > maxVL) {

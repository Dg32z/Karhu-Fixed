...
public final class KarhuPlayer {
    ...
    private final Map<Integer, TaskData> tasks = new LinkedHashMap<Integer, TaskData>();
    ...
    public Map<Short, ObjectArrayList<Consumer<Short>>> waitingConfirms = new HashMap<Short, ObjectArrayList<Consumer<Short>>>();
    ...
    public short lastPostId = (short)-100;
    public final Map<Integer, ConcurrentLinkedDeque<VelocityPending>> velocityPending = new HashMap<Integer, ConcurrentLinkedDeque<VelocityPending>>();
    ...
    private List<Integer> lastTargets = new ArrayList<Integer>();
    ...
    private List<Double> eyesLegacy = Arrays.asList(Float.valueOf(1.54f), Float.valueOf(1.62f)).stream().map(Double::valueOf).collect(Collectors.toList());
    ...
    public final Map<Short, Long> transactionTime = new ConcurrentHashMap<Short, Long>();
    ...
    public final List<Pair<Integer, Integer>> exemptMap = new ArrayList<Pair<Integer, Integer>>();
    public final Map<SubCategory, Pair<Integer, Integer>> exemptCategoryMap = new HashMap<SubCategory, Pair<Integer, Integer>>();
    private double cps;
    ...
    public final Map<Integer, Deque<Integer>> backSwitchSlots = new HashMap<Integer, Deque<Integer>>();
    ...
    public KarhuPlayer(UUID uuid, Karhu karhu, long now) {
        ...
        this.bukkitPlayer = Bukkit.getPlayer((UUID)uuid);
        this.entityId = this.bukkitPlayer.getEntityId();
        this.transactionId = Short.MIN_VALUE;
        this.checkViolationTimes = new HashMap<Check, Set<Long>>();
        this.checkVlMap = new HashMap<Check, Double>();
        this.lastLastLastLocation = this.lastLastLocation = (this.lastLocation = (this.location = new CustomLocation(0.0, 0.0, 0.0, 0.0f, 0.0f)));
        this.checkManager = new CheckManager(this, karhu);
        this.gameMode = GameMode.getById(this.bukkitPlayer.getGameMode().getValue());
        User user2 = user = this.bukkitPlayer != null ? PacketEvents.getAPI().getPlayerManager().getUser(this.bukkitPlayer) : null;
        if (user != null) {
    ...
    public void handleKickAlert(String type) {
        if (!this.kicked) {
            ...
            MiscellaneousAlertPoster.postMisc(Karhu.getInstance().getConfigManager().getAntiCrashMessage().replaceAll("%debug%", type).replaceAll("%player%", this.bukkitPlayer.getName()), this, "Crash");
    ...
    public void checkVelocity() {
        for (Map.Entry<Integer, ConcurrentLinkedDeque<VelocityPending>> myMapEntry : this.velocityPending.entrySet()) {
            for (VelocityPending velocityCheck : myMapEntry.getValue()) {
                double min;
                Vector velocity = velocityCheck.getVelocity();
                if (velocityCheck.isMarkedSent()) continue;
                long nanoStart = System.nanoTime();
                double ogHz = MathUtil.hypot(velocity.getX(), velocity.getZ());
                double d = min = this.getClientVersion().getProtocolVersion() > 47 ? 0.003 : 0.005;
                if (this.attacks != 0) {
                    double precisionH;
                    ...
                    Vector knockbackVector = new Vector(kbs.getX().doubleValue(), kbY, kbs.getY().doubleValue());
                    ...
                    double d2 = precisionH = this.elapsed(this.lastCollidedH) <= 2 ? Math.max(ogHz, horizontalDistance) : 0.03125;
                    ...
                    Vector knockbackVector = new Vector(sheesh.getX().doubleValue(), kbY, sheesh.getY().doubleValue());
                    Vector playerVector = new Vector(this.deltas.deltaX, this.deltas.motionY, this.deltas.deltaZ);
                    double horizontalDistance = MathUtil.horizontalDistance(knockbackVector, playerVector);
                    double verticalDistance = MathUtil.verticalDistance(knockbackVector, playerVector);
                    double precisionY = this.elapsed(this.lastCollidedV) <= 2 ? (double)0.43f : 0.03125;
                    double d3 = precisionH = this.elapsed(this.lastCollidedH) <= 2 ? Math.max(ogHz, horizontalDistance) : 0.031;
    ...
    public void useOldTransaction(Consumer<Short> callback, short uid) {
        ObjectArrayList map = this.waitingConfirms.computeIfAbsent(uid, k -> new ObjectArrayList());
        map.add(callback);
        this.waitingConfirms.put(uid, (ObjectArrayList<Consumer<Short>>)map);
    }
        ...
        if (this.entityId != -1) {
            for (World world : Bukkit.getWorlds()) {
                for (Entity entity : SpigotReflectionUtil.getEntityList(world)) {
                    if (entity.getEntityId() != this.entityId) continue;
                    return world;
                }
            }
        }
        return Bukkit.getWorld((UUID)((World)Bukkit.getWorlds().get(0)).getUID());
    }
    ...
    public void velocityTick(Vector vector) {
        ...
        int velocityV = (int)Math.ceil(FastMath.pow((double)(Math.abs(vector.getY()) + 2.0), (int)2)) * 2;
    ...
    public short getNextTransactionIdSilent() {
        ...
        if ((predict = (short)(predict + 1)) > -20001) {
            predict = Short.MIN_VALUE;
        }
        return (short)predict;
    }
    ...
    public boolean couldBeUnloadedClient() {
        return this.elapsed(this.getLastInUnloadedChunk()) <= MathUtil.getPingInTicks(this.getTransactionPing()) + 2 || this.elapsed(this.getLastWorldChange()) <= MathUtil.getPingInTicks(this.getTransactionPing()) + 10 || this.elapsed(this.getLastPossibleInUnloadedChunk()) <= 2;
    }
